...
const audio_format_t *audio_i2s_setupx(const audio_format_t *intended_audio_format,
...
//     uint32_t divider = system_clock_frequency * 4 / producer->format->sample_freq; // avoid arithmetic overflow
    uint32_t divider = system_clock_frequency * 32 / (producer->format->sample_freq * 10); // avoid arithmetic overflow
    pio_sm_set_clkdiv_int_frac(audio_pio, shared_state.pio_sm, divider >> 8u, divider & 0xffu);
