...
static void init_jamma() {
  ...
  joydata[0] = joydata[1] = 0;
  joydata_usb[0] = joydata_usb[1] = 0;
  reload_data = 0;
  joydata_md[0] = joydata_md[1] = 0;
...
static uint8_t db9_translate(uint8_t d) {
  uint8_t ret = (d >> 4) | ((d & 8) << 1) | ((d & 4)) << 3;
  return ret == 0x3f ? 0x00 : ret;
}
...
void jamma_Kill() {
  ...
  pio_set_irq0_source_enabled(jamma_pio, pis_interrupt0, false);
  irq_set_enabled (JAMMA_PIO_IRQ, false);
#ifdef JAMMA_JAMMA
...
#define debuga(a) printf a
void jamma_SetData(uint8_t inst, uint32_t data) {
  debuga(("jamma_SetData: inst %d data %08X\n", inst, data));
  debug(("jamma_SetData: inst %d data %08X\n", inst, data));
  if (inited == 1) {
    joydata_usb[inst] = data & 0xff;
    // reload_data = ~((joydata_usb[1] << 8) | joydata_usb[0]);
    reload_data = (joydata_usb[1] << 8) | joydata_usb[0];
  }
 }
